...
public class MainActivity extends AppCompatActivity {
    ...
    private Button btnMenuTrim, btnMenuLoop, btnMerge, btnRemux; 
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnMenuTrim = findViewById(R.id.btnMenuTrim);
        btnMenuLoop = findViewById(R.id.btnMenuLoop);
        ...
        btnMenuTrim.setOnClickListener(v -> {
            if (selectedPaths.isEmpty()) { showToast("Pilih video terlebih dahulu."); return; }
            showTrimBottomSheet();
        });
        
        btnMenuLoop.setOnClickListener(v -> {
            if (selectedPaths.isEmpty()) { showToast("Pilih video terlebih dahulu."); return; }
            showLoopBottomSheet(); 
        });

        btnMerge.setOnClickListener(v -> {
            if (selectedPaths.size() < 2) { showToast("Minimal 2 video diperlukan untuk fitur ini."); return; }
            processMerge();
        });
    ...
    private void showTrimBottomSheet() {
        ...
        EditText etTrimSegment = dialog.findViewById(R.id.etTrimSegment);
        EditText etTrimStart = dialog.findViewById(R.id.etTrimStart);
        EditText etTrimEnd = dialog.findViewById(R.id.etTrimEnd);
        Button btnTrimWithAudio = dialog.findViewById(R.id.btnTrimWithAudio);
        Button btnTrimNoAudio = dialog.findViewById(R.id.btnTrimNoAudio);
        ...
        View.OnClickListener trimListener = v -> {
            boolean keepAudio = (v.getId() == R.id.btnTrimWithAudio);
            boolean isAutoSplit = (rgTrimMode.getCheckedRadioButtonId() == R.id.rbTrimAuto);
            
            if (isAutoSplit && etTrimSegment.getText().toString().isEmpty()) {
                showToast("Isi durasi part!"); return;
            } else if (!isAutoSplit && (etTrimStart.getText().toString().isEmpty() || etTrimEnd.getText().toString().isEmpty())) {
                showToast("Isi Start dan End!"); return;
            }

            String segmentVal = etTrimSegment.getText().toString();
            String startVal = etTrimStart.getText().toString();
            String endVal = etTrimEnd.getText().toString();

            dialog.dismiss();
            processTrim(keepAudio, isAutoSplit, segmentVal, startVal, endVal); 
        };

        if (btnTrimWithAudio != null) btnTrimWithAudio.setOnClickListener(trimListener);
        if (btnTrimNoAudio != null) btnTrimNoAudio.setOnClickListener(trimListener);
        
        dialog.show();
    }

    private void showLoopBottomSheet() {
        BottomSheetDialog dialog = new BottomSheetDialog(this);
        ...
        if (btnExecuteLoop != null) {
            btnExecuteLoop.setOnClickListener(v -> {
                if (etLoopDuration.getText().toString().isEmpty()) {
                    showToast("Isi target durasi terlebih dahulu!");
                    return;
                }
                ...
                dialog.dismiss();
                processButterLoop(modeId, targetDur);
            });
        }
        dialog.show();
    }

    private void processTrim(boolean keepAudio, boolean isAutoSplit, String segmentVal, String startVal, String endVal) {
                ...
                for (int i = 0; i < selectedPaths.size(); i++) {
                    ...
                    if (isAutoSplit) {
                        ...
                        while (currentStart < totalDur) {
                            ...
                            runOnUiThread(() -> updateStatus("✂️ [File " + fileIndex + "] Ekstraksi Part " + currentPart + "..."));
                        ...
                        double d_start = parseTimeToSeconds(startVal);
                        double d_end = parseTimeToSeconds(endVal);

                        if (d_start >= d_end || d_start > totalDur) throw new Exception("Waktu Start tidak valid.");

                        double diff = d_end - d_start;
                        runOnUiThread(() -> updateStatus("✂️ [File " + fileIndex + "] Ekstraksi Custom Trim..."));
    ...
    private void processButterLoop(int modeId, int targetDur) {
        ...
        if (resultCode == RESULT_OK && data != null) {
            ...
            new Thread(() -> {
                ...
                runOnUiThread(() -> {
                    ...
                    if (selectedPaths.size() == 1) setupPreviewPlayer(selectedPaths.get(0));
                });
            }).start();
        }
    }
    ...
    private void setupPreviewPlayer(String path) {
        ...
        videoPreview.setOnPreparedListener(mp -> {
            ...
            btnPlayPause.setOnClickListener(v -> {
                if (videoPreview.isPlaying()) {
                    ...
                    if (videoPreview.getCurrentPosition() >= duration - 500) videoPreview.seekTo(0);
                    videoPreview.start();
        ...
        seekBarPreview.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if (fromUser) tvPreviewTime.setText(formatTimeString(progress) + " / " + formatTimeString(videoPreview.getDuration()));
            }
            @Override
    ...
    private void moveToPublicFolder(String srcPath, String destDir, String name) throws Exception {
        ...
        try (InputStream in = new FileInputStream(s); OutputStream out = new FileOutputStream(d)) {
            byte[] b = new byte[4096]; int l; while ((l = in.read(b)) > 0) out.write(b, 0, l);
            s.delete();
        } catch (Exception e) { throw new Exception("Transfer I/O diblokir sistem. Detail: " + e.getMessage()); }
    }
    ...
    private void setLoading(boolean b) {
        ...
        btnMenuTrim.setEnabled(!b); btnMenuLoop.setEnabled(!b); 
        btnMerge.setEnabled(!b); btnRemux.setEnabled(!b);
